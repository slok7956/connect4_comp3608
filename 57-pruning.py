...
def recurse(state, turn, score, alpha, beta, depth, max_depth=-1):
    ...
    next_boards={k:change_state(state,turn,k) for k in range(0,7)}
    next_scores={k:evaluation(next_boards[k]) for k in range(0,7)}
    for check in next_boards.keys():
        if turn=="red" and next_scores[check] > beta:
            return score
        if turn=="yellow" and next_scores[check] < alpha:
            return score
    
    fin_dict.update({k:recurse(next_boards[k], next_turn, next_scores[k], alpha, beta, depth-1) for k in range(0,7)})                               
    #choose highest-value node
    if turn == "red":
