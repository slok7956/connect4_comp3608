...
def recurse(state, turn, score, alpha, beta, depth, max_depth=-1):
    ...
    sc={c:evaluation(st[c]) for c in range(0,7)}
    a_b={0:a_b_update(next_turn,sc[0],alpha,beta)}
    for c in range(1,7):
      a_b.update({c:a_b_update(next_turn,sc[c],a_b[c-1][0],a_b[c-1][1])})
    ...
    #expand next layer
    for i in range(0,7):
      fin_dict.update({i:recurse(st[i], next_turn, sc[i], a_b[i][0], a_b[i][1], depth-1)})
