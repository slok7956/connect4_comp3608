...
def recurse(state, turn, score, alpha, beta, depth, max_depth=-1):
    recurse.counter += 1
    #alpha & beta: values inherited from parent node
    #g_a & g_b: values edited & passed between children
    global g_a,g_b
    g_a=alpha
    g_b=beta    
    ...
    while i < 7:
        ...
        if turn=="red" and score>g_a:
          g_a=score
        if turn=="yellow" and score<g_b:
          g_b=score
        ...
        #if prune(next_turn,next_score,alpha,beta):
        if turn=="red" and (g_a>=beta):
          i+=1
          continue
        elif turn=="yellow" and (g_b<=alpha):
          i+=1
          continue
          return score
        else:
          ...
          alpha_copy=g_a
          beta_copy=g_a
          print(next_score,alpha_copy,beta_copy)
          level_dict[i]=(next_state,next_score,alpha_copy,beta_copy)
