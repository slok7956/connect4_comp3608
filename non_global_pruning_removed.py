...
def connect_four_ab(contents, turn, max_depth):
    ...
    beta=float('inf')
    recurse.counter = 0
    result = recurse(contents,turn,alpha,beta,max_depth,max_depth)
    return f"{result}\n{recurse.counter}"
...
def recurse(state, turn, alpha, beta, depth, max_depth=-1):
    recurse.counter += 1
    ...
    if (score == 10000 and turn == "yellow") or (score == -10000 and turn == "red"):
        return score
    #after calculating score, before exploring another level
    #if prune(turn,score,alpha,beta):
        #return score
    ...
    while i < 7:
        ...
        fin_dict[i].append(recurse(next_state, next_turn, alpha, beta, next_depth))
        i = column
        i += 1
